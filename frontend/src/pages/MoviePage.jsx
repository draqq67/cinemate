...
function CommentItem({ comment, onReply, depth = 0 }) {
  ...
  return (
    <div style={{
      ...
      borderLeft: depth > 0 ? '2px solid var(--color-border-tertiary)' : 'none',
...
export default function MoviePage() {
  ...
  const handleRate = async (score) => {
    ...
    try {
      ...
      setData(d => ({
        ...d,
        movie: { ...d.movie, avg_rating: updated.avg_rating, rating_count: updated.rating_count },
      }));
    } catch (err) {
      console.error(err);
    } finally {
  ...
  const saveProgress = useCallback(async (progressSeconds) => {
    try {
      await client.post(`/movies/${tmdbId}/progress`, { progress_s: progressSeconds });
    } catch (err) {
      console.error(err);
    }
  }, [tmdbId]);
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--color-background-primary)' }}>
      ...
      {movie.backdrop_path && (
        <div style={{ width: '100%', height: '340px', overflow: 'hidden', position: 'relative' }}>
          ...
          <div style={{
            ...
            background: 'linear-gradient(to bottom, transparent 40%, var(--color-background-primary))',
          }} />
        </div>
      )}
      ...
      <div style={{
        maxWidth: '1000px',
        margin: movie.backdrop_path ? '-180px auto 0' : '0 auto',
        padding: '0 24px 60px',
        ...
        <div style={{ display: 'flex', gap: '28px', marginBottom: '32px', alignItems: 'flex-start' }}>
          {movie.poster_path && (
            <img
              ...
              alt={movie.title}
              style={{
                width: '180px', borderRadius: '10px', flexShrink: 0,
                border: '0.5px solid var(--color-border-tertiary)',
              }}
            />
          )}

          <div style={{ flex: 1, paddingTop: movie.backdrop_path ? '0' : '0' }}>
            <h1 style={{ fontSize: '28px', fontWeight: 500, marginBottom: '4px' }}>
              {movie.title}
            </h1>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px', marginBottom: '14px' }}>
              {movie.year && (
                <span style={{ fontSize: '13px', color: 'var(--color-text-secondary)' }}>{movie.year}</span>
              )}
              {movie.runtime && (
              ...
              {movie.genres?.map(g => (
                <Link
                  key={g}
                  to={`/browse?genre=${g}`}
                  style={{ fontSize: '11px', padding: '2px 8px', borderRadius: '10px', background: '#E6F1FB', color: '#0C447C', textDecoration: 'none' }}
                >
                  {g}
                </Link>
              ))}
            ...
            <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
              ...
              {movie.jellyfin_id ? (
                <button
                  ...
                  style={{
                    padding: '9px 20px', background: '#185FA5', border: 'none',
                    color: '#fff', borderRadius: '6px', fontSize: '13px',
                    fontWeight: 500, cursor: streamLoading ? 'not-allowed' : 'pointer',
              ...
              {user ? (
                <button
                  ...
                  style={{
                    padding: '8px 16px',
                    border: `0.5px solid ${inWatchlist ? '#85B7EB' : 'var(--color-border-secondary)'}`,
                    borderRadius: '6px',
                    background: inWatchlist ? '#E6F1FB' : 'transparent',
                    color: inWatchlist ? '#0C447C' : 'var(--color-text-secondary)',
              ...
              <div key={c.id} style={{ flexShrink: 0, width: '80px', textAlign: 'center' }}>
                {c.profile_path
                  ? <img
                      ...
                      alt={c.name}
                      style={{ width: '80px', height: '80px', borderRadius: '50%', objectFit: 'cover', border: '0.5px solid var(--color-border-tertiary)' }}
                    />
                  : <div style={{
                      ...
                      fontSize: '18px', fontWeight: 500, color: 'var(--color-text-tertiary)',
                    }}>
                      {c.name.slice(0, 1)}
                    </div>
                }
                <div style={{ fontSize: '11px', fontWeight: 500, marginTop: '5px', color: 'var(--color-text-primary)' }}>
                  {c.name}
                </div>
        ...
        {/* Tabs */}
        <div style={{ borderBottom: '0.5px solid var(--color-border-tertiary)', display: 'flex', marginBottom: '24px' }}>
          {tabs.map(t => (
            <button
              key={t.id}
                  ...
                  <div key={r.id} style={{ padding: '16px 0', borderBottom: '0.5px solid var(--color-border-tertiary)' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '8px' }}>
                      <div style={{
                        ...
                        {r.avatar_path
                          ? <img
                              src={r.avatar_path.startsWith('/https') ? r.avatar_path.slice(1) : `${PROFILE}${r.avatar_path}`}
                              alt=""
